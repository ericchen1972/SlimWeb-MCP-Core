...
'Theme create flow: require a name, call slimweb_themes_list to check custom theme duplicates, stop on an exact duplicate, call slimweb_site_theme_mode_get and optionally slimweb_site_theme_mode_update for dark/neon/high-contrast requests, call slimweb_themes_create_from_default, call slimweb_theme_shell_get_context, call slimweb_design_context_get, design navbar/floating_actions/footer/root CSS/body background/overall visual atmosphere from user intent, site colors, shell reference, and framework, follow the Theme design rule, call slimweb_themes_update_root_elements without changing page body content, call slimweb_theme_style_profile_upsert, return the theme name and theme_id, and only call slimweb_themes_activate when the user explicitly asks to activate it. Creating from Default does not copy stored Default root elements; untouched slots use the canonical runtime fallback.',
'Theme-managed edit flow: navbar, floating_actions, footer, root CSS, and style profile are managed by Theme. Always call slimweb_design_context_get first and follow theme_edit_policy. Default is immutable: when the active Theme is Default, create a new Theme with slimweb_themes_create_from_default and apply changes there without asking whether to edit Default. When the active Theme is custom, ask whether to create a new Theme or modify the current Theme. If the user chooses a new Theme, clone the active non-Default Theme with slimweb_themes_create_from_theme. If the user explicitly chooses direct modification, pass confirmed_active_theme_edit=true to every root-element or style-profile write. Resolve the exact custom theme_id, read its style profile and shell context, preserve Theme runtime boundaries, and only activate a Theme when the user explicitly asks.',
...
const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Create a new non-Default theme/page style scheme. This does not copy any Default root elements or style profile; untouched shell slots use the canonical runtime fallback. The theme acts as the base styling layer for every page, including the homepage. Do not choose light/dark here; the style scheme inherits site-level color mode.',
    inputSchema: {
      type: 'object',
      properties: {
        site_id: {
  ...
  {
    name: 'slimweb_themes_create_from_theme',
    description: 'Create a new inactive theme by cloning the shell, root assets, and style profile of an explicit non-Default source Theme. Page body content is not copied. Use only after the user chooses to create a new Theme from the current custom Theme.',
    inputSchema: {
      type: 'object',
      properties: {
        site_id: { type: 'integer', description: 'Target SlimWeb site ID.' },
        source_theme_id: { type: 'integer', description: 'Existing non-Default source Theme ID.' },
        name: { type: 'string', description: 'Human-readable name for the new Theme.' }
      },
      required: ['site_id', 'source_theme_id', 'name']
    }
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        confirmed_active_theme_edit: {
          type: 'boolean',
          description: 'Required true only when the target is the active custom Theme and the user explicitly chose direct modification.'
        }
      },
      required: ['site_id', 'theme_id']
    }
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        confirmed_active_theme_edit: { type: 'boolean' }
      },
      required: ['site_id', 'theme_id']
    }
  },
...
const TOOL_PERMISSION_RULES = {
  ...
  slimweb_themes_create_from_theme: ['page_management', 'page_management_templates'],
...
async function toolResultForCall(message, request, context) {
  ...
  switch (name) {
    ...
    case 'slimweb_themes_create_from_theme': {
      try {
        const result = await context.accountRepository.createThemeFromTheme(
          await actorForTool(session, name, toolArgs(message), context),
          toolArgs(message)
        );
